...
bool AArch64RegisterInfo::getRegAllocationHints(
  ...
  for (auto &Def : MRI.def_instructions(VirtReg)) {
    ...
    if (!Dst.getSubReg() || !VRM->hasPhys(Src.getReg()))
      continue;

    // Find the ZPR register mapped to the source of the copy.
    MCPhysReg SrcZPR = VRM->getPhys(Src.getReg());
    if (Src.getSubReg())
      SrcZPR = getSubReg(SrcZPR, Src.getSubReg());
  ...
  for (MachineInstr &MI : MRI.def_instructions(VirtReg)) {
    if (MI.getOpcode() != AArch64::COPY_INTO_TRANSPOSED_TUPLE)
      return TargetRegisterInfo::getRegAllocationHints(VirtReg, Order, Hints,
                                                       MF, VRM);
    ...
    Register FirstOpVirtReg = AArch64::NoRegister;
    for (const MachineInstr &Def :
         MRI.def_instructions(MI.getOperand(0).getReg())) {
      if (Def.getOpcode() == MI.getOpcode() &&
          Def.getOperand(0).getSubReg() == AArch64::zsub0) {
        FirstOpVirtReg = Def.getOperand(1).getReg();
        break;
      }
    }

    // Look up the physical register mapped to the first operand of the pseudo.
    if (FirstOpVirtReg == AArch64::NoRegister || !VRM->hasPhys(FirstOpVirtReg))
      continue;
