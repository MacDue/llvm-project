...
TargetStackID::Value
AArch64FrameLowering::getStackIDForScalableVectors(bool IsPred) const {
  return IsPred ? TargetStackID::ScalablePredVector
...
void AArch64FrameLowering::emitPrologue(MachineFunction &MF,
  ...
  StackOffset SVELocalsSize = getZPRStackSize(MF) + getPPRStackSize(MF);
  ...
  if (EmitCFI && !EmitAsyncCFI) {
    if (HasFP) {
      ...
      StackOffset TotalSize =
          getZPRStackSize(MF) + getPPRStackSize(MF) +
...
static void determineSVEStackObjectOffsets(MachineFunction &MF,
  ...
  auto processCalleeSaves = [&](int Min, int Max, int64_t &Offset) {
    for (int I = Min; I <= Max; ++I) {
      ...
      if (AssignOffsets) {
        LLVM_DEBUG(dbgs() << "FI: " << I << ", Offset: " << -Offset << "\n");
        Assign(I, -Offset);
      }
    }
  };
...
StackOffset AArch64FrameLowering::getFrameIndexReferencePreferSP(
  ...
  if (MFI.hasVarSizedObjects() ||
      MF.getInfo<AArch64FunctionInfo>()->getStackSizeZPR() ||
      MF.getInfo<AArch64FunctionInfo>()->getStackSizePPR() ||
