...
struct FunctionInfo {
  ...
  bool UsesZAorZT0 = false;
  bool ClobbersZAorZT0 = false;
};
...
FunctionInfo MachineSMEABI::collectNeededZAStates(SMEAttrs SMEFnAttrs) {
  ...
  for (MachineBasicBlock &MBB : *MF) {
    ...
    for (MachineInstr &MI : reverse(MBB)) {
      ...
      if (!MI.isReturn()) {
        UsesZAorZT0 |= NeededState == ACTIVE || NeededState == ACTIVE_ZT0_SAVED;
        ClobbersZAorZT0 |= NeededState != ACTIVE;
      }
    }
  ...
  return FunctionInfo{std::move(Blocks), AfterSMEProloguePt,
                      PhysLiveRegsAfterSMEPrologue, UsesZAorZT0,
                      ClobbersZAorZT0};
}
...
bool MachineSMEABI::runOnMachineFunction(MachineFunction &MF) {
  ...
  // Private ZA with no use of ZA/ZT0 => Do nothing.
  // Shared/agnostic ZA with no clobbers of ZA/ZT0 => Do nothing.
  if ((AFI->getSMEFnAttrs().hasPrivateZAInterface() && !FnInfo.UsesZAorZT0) ||
      (!AFI->getSMEFnAttrs().hasPrivateZAInterface() &&
       !FnInfo.ClobbersZAorZT0)) {
    return false;
  }
