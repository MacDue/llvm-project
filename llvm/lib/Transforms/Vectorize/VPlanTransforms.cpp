...
void VPlanTransforms::materializeVFAndVFxUF(VPlan &Plan, VPBasicBlock *VectorPH,
  ...
  if (ClampedVF) {
    ...
    VF.replaceAllUsesWith(ClampedVF);
    VFxUF.replaceAllUsesWith(ClampedVF);
    return;
  }
...
VPlanTransforms::materializeAliasMask(VPlan &Plan, VPBasicBlock *AliasCheck,
  ...
  VPValue *PopCount = Builder.createNaryOp(VPInstruction::PopCount, {Mask});
  return Builder.createScalarZExtOrTrunc(PopCount, IVTy, I64Ty,
                                         DebugLoc::getCompilerGenerated());
}
