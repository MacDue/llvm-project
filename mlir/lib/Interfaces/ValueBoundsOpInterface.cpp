...
FailureOr<int64_t> ValueBoundsConstraintSet::computeConstantBound(
  ...
  int64_t pos =
      cstr.populateConstantBoundSet(type, map, operands, stopCondition);
...
int64_t ValueBoundsConstraintSet::populateConstantBoundSet(
    presburger::BoundType type, Value value, std::optional<int64_t> dim,
  ...
  AffineMap map =
      AffineMap::get(/*dimCount=*/1, /*symbolCount=*/0,
                     Builder(value.getContext()).getAffineDimExpr(0));
  return populateConstantBoundSet(type, map, {{value, dim}}, stopCondition);
}
