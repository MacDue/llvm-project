// RUN: mlir-opt %s \
// RUN:    \
// RUN:    -arm-sme-vector-legalization -cse -canonicalize \
...
//   %c16 = arith.constant 4 : index
//   %svl_w = arm_sme.streaming_vl <word>
//   %size_w = arith.muli %c16, %svl_w : index
//   %size_h = arith.muli %c1, %svl_w : index
//   %out = memref.alloca(%size_w, %size_h) : memref<?x?xf32>

//   %in = memref.alloca(%size_h, %size_w) : memref<?x?xf32>
//   scf.for %i = %c0 to %size_h step %c1 {
//     scf.for %j = %c0 to %size_w step %c1 {
//       %ff = arith.index_cast %i : index to i32
//       %f = arith.sitofp %ff : i32 to f32
//       memref.store %f, %in[%i, %j] : memref<?x?xf32>
//     }
//   }

//   %pIn = memref.cast %in : memref<?x?xf32> to memref<*xf32>
//   call @printMemrefF32(%pIn) : (memref<*xf32>) -> ()
//   vector.print str "-----"

//   %pad = arith.constant 0.0 : f32
//   %a = arith.constant 4 : index
//   %b = arith.constant 30 : index
//   %mask = vector.create_mask %a, %b : vector<[4]x[16]xi1>
//   %0 = vector.transfer_read %in[%c0, %c0], %pad, %mask {permutation_map = #transpose, in_bounds = [true, true]} : memref<?x?xf32>, vector<[16]x[4]xf32>
//   vector.transfer_write %0, %out[%c0, %c0] {in_bounds = [true, true]} : vector<[16]x[4]xf32>, memref<?x?xf32>

//   %pOut = memref.cast %out : memref<?x?xf32> to memref<*xf32>
//   call @printMemrefF32(%pOut) : (memref<*xf32>) -> ()
...
func.func @main() {
  ...
  %c16 = arith.constant 2 : index
  %svl_w = arm_sme.streaming_vl <word>
  %size_w = arith.muli %c16, %svl_w : index
  %size_h = arith.muli %c16, %svl_w : index
  %out = memref.alloca(%size_w, %size_h) : memref<?x?xf32>
  ...
  %b = arith.constant 30 : index
  // %mask = vector.create_mask %a, %b : vector<[4]x[16]xi1>
  %0 = vector.transfer_read %in[%c0, %c0], %pad  {in_bounds = [true, true]} : memref<?x?xf32>, vector<[8]x[8]xf32>
  vector.transfer_write %0, %out[%c0, %c0] {permutation_map = #transpose, in_bounds = [true, true]} : vector<[8]x[8]xf32>, memref<?x?xf32>
